...
class StockTradingEnv(gym.Env):
    ...
    def reset(self, *_args, **kwargs) -> tuple[np.array, dict[str, float]]:
        ...
        **kwargs : Any
        ...
        self.action_space.seed(
            int(np.random.uniform(0, seed if seed is not None else 1))
        )
        self._reset_episode()
    ...
    def render(self, mode: str | None = None) -> None:
        ...
        assert (
            pause_time >= 0.0
        ), "Negative pause time - reduce fps or reduce rendering time"
    ...
    def get_observation(self) -> np.array:
        ...
        if len(observation) < self.window_size:
            padding = np.zeros(
                (self.window_size - len(observation), self.signal_features.shape[1])
            )
    ...
    def update_profit(self, action: int) -> None:
        ...
        if trade_occurred or self.truncated:
            ...
            if self.position == Positions.LONG:
                shares = (
                    self.total_profit * (1 - self.trade_fee_ask_percent)
                ) / last_trade_price
                self.total_profit = (
        ...
        while current_step < len(self.prices):
            ...
            if self.prices[current_step] < self.prices[current_step - 1]:
                while (
                    ...
                    and self.prices[current_step] < self.prices[current_step - 1]
                ):
                    current_step += 1
